...
const FavoritesButton = () => {
    ...
    useEffect(() => {
        setIsFavoritesPage(location.pathname === '/favorites');
    }, [location.pathname]);

    return (
        <div  onClick={() => navigate('/user/favorites')} className="text-amber-950 cursor-pointer transition-all duration-300 ease-in-out hover:scale-[1.1]">
            {isFavoritesPage ? (
                <svg onClick={() => navigate('/favorites')} xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34" fill="none">
