    ...
    return (
        <div className={`${className}`}>
            <div className={` button-menu`} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} >
                <SettingFilled className="text-adaptive-icons cursor-pointer z-50" style={{ backgroundColor: "transparent", color: "white" }} />
                <div className={`flex flex-col gap-[2vh] button-menu-container z-0 ${open !== undefined ? !open ? 'close' : 'open' : ''}`}>
                    {isEdit &&
                        <svg className="h-full w-auto cursor-pointer " onClick={() => { navigate(`advert/edit/${id}`) }} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
