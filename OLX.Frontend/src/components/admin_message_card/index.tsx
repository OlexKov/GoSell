...
const AdminMessageCard: React.FC<AdminMessageCardProps> = ({ adminMessage, divider, className, dividerClassName, onDelete, onClick }) => {
    ...
    return (
        <div className={`${onClick ? 'cursor-pointer' : ''}`} onClick={() => onClick && !adminMessage.readed && onClick(adminMessage.id)}>
            <div className={`min-h-[70px] ${className}`}>
                <div className={`flex gap-[1vw] h-full  items-center rounded-md ${adminMessage.readed ? 'bg-white' : 'bg-slate-100'}`}>
                    <div className=" h-full aspect-square p-[0.5vh] bg-white rounded-md border border-[#9B7A5B]">
                        <img className="h-full aspect-square" src={Images.logo} />
                    </div>
                    ...
                    {onDelete &&
                        <svg onClick={deleteClick} className="h-[2vh] min-h-[14px] aspect-square cursor-pointer mr-[1vw] ml-auto" xmlns="http://www.w3.org/2000/svg" width="18" height="20" viewBox="0 0 18 20" fill="none">
